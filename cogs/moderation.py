...
class ModerationCog:
    ...
    async def temprole(self, ctx: commands.Context, user: discord.Member, rolename:str, durationNumber: int, durationIdentifier: str, *):
            ...
            if roleid is 0:
                ...
                if role is None:
                    ...
                    await BugLog.logToModLog(ctx.guild,
                                                     f":zipper_mouth: {target.name}#{target.discriminator} (`{target.id}`) has been muted by {ctx.author.name} for {reason}")
